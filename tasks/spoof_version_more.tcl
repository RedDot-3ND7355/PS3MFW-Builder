...
# Modified By RazorX & RedDot-3ND7355
...
namespace eval ::spoof_version_more {
    ...
    array set ::spoof_version_more::options {
	  ...
	  --spoof-label3 "                                      The Version, Build & Date Of Your CFW.                                     : :"
	  --spoof-label4 "                                                                                                                                                   : :"
      --spoof "4.46 60826 001:CEX-ww 20130620"
	  --spoof-label5 "--------------------------------------------------------------------------------------------------------------   : :"
    }
    ...
    proc main {} {
      ...
      if {$release != "" || $build != "" || $target != "" || $auth != ""} {
		...
		::modify_devflash_file2 [file join dev_flash vsh etc version.txt] ::spoof_version_more::version_txt
		}
	  }
      if {$build != "" || $auth != ""} {
		...
		::modify_devflash_file2 [file join dev_flash vsh module vsh.self] ::spoof_version_more::patch_self
		}
	  }
      if {$build != "" && $bdate != ""} {
    ...
    proc patch_self {self} {
	  ...
	  ::modify_self_file2 $self ::spoof_version_more::patch_elf
	  }
	}
...
# catch_die {::patch_elf $elf $search 4 $replace} "Unable to patch self [file tail $elf]"
    if { ${::SUF} == "4.50" } {
    log "Skipping due to fw not compatible with these patterns!"
	} else {
